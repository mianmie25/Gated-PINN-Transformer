...
class FatigueDataset(Dataset):
    def __init__(self, data_dir, sample_ids=None):
        ...
        self.features = np.array(self.features, dtype=np.float32)  # (n_samples, 1000, 1)
...
class GatedAttention(nn.Module):
    def __init__(self, d_model, dropout_rate=config['dropout_rate']):
        super().__init__()
        self.attention = nn.MultiheadAttention(d_model, num_heads=1, batch_first=False)
...
def train_model(model, train_loader, test_loader, config):
    ...
    for epoch in range(config['epochs']):
        ...
        for batch_feat, batch_label in train_loader:
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # 梯度裁剪防过拟合
            optimizer.step()
...
def calculate_feature_contribution(model, test_loader, config):
    ...
    for batch_feat, batch_label in test_loader:
        ...
        # 移除整体torch.no_grad()，仅对注意力权重的提取使用no_grad
        # 前向传播获取输出和注意力权重
